	...
	<?php	else : get_template_part( 'template-parts/content', 'none' );?>
		...
		<div class="container">
			<?php $search = get_search_query();?>
            	<h1>Resultados para "<?php echo $search?>"</h1>
					<div class="cabecera-buscador">
						<p><?php printf( esc_html__( 'Se han encontrado %s resultados para el termino %s en todos los apartados', 'chilevalora' ),
					 	'<strong>' . total_results() . '</strong>', '<strong>' . get_search_query() . '</strong>'  );?></p>
					</div>
				</div>
