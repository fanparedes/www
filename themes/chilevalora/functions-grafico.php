...
function contratos_indefinidos_subsectores( $data ) {
    ...
    if(is_numeric($id_sector)){
        $sql = "
            select a.ano, a.id_sector, a.name_sector, a.sub_sector, a.name_subsector, round(cast(((a.contratados::float*100)/b.total::float) as numeric), 1) as porcentaje
            from (select cs.id_sector, cs.name_sector, cc.id_class as sub_sector, cc.name_class as name_subsector, sum(ca.number_workers::float) as contratados, ca.ano
        ...
        if(is_array($rs) && count($rs)>0){
            foreach ($rs as $row) {
                ...
                {
                    if($result[$i]['name']==$name_subsector)
                    {
                        array_push($result[$i]['data'], $porcentaje);
                        $repeat=true;
                        break;
                    }
                }
                if($repeat==false){
                    $result[] = array('name' => $name_subsector, 'data' => array($porcentaje));
                }
            }
            $rs_response = (array) $result;
...
add_action( 'rest_api_init', function () {
    register_rest_route( 'wp/v2', '/contratos_indefinidos_subsectores/(?P<id_sector>\d+)', array(
        'methods' => 'GET',
...
function contratos_indefinidos_sectores_variacion_dashboard(  ) {
    ...
    $sql = "
        select
            sec2.id_sector,
            sec2.name_sector,   
            (select max(ano::int) from cl_afc) as ano,
            (select variacion_sector(sec2.id_sector, (select max(ano::int) from cl_afc))) as presente,
            (select variacion_sector(sec2.id_sector, (select max(ano::int)-1 from cl_afc))) as pasado
        from cl_class clss 
        inner join cl_subclass sbca on (clss.id_class = sbca.id_class)
        inner join cl_groups grp on (clss.id_group = grp.id_group)
        inner join cl_divisions divs on (grp.id_division = divs.id_division)
