...
function enqueue_ajaxjs() {
  ...
  //       wp_enqueue_script('highcharts_js', get_template_directory_uri() . '/js/highcharts.js', array(), '3.3.4');
		...
		// wp_enqueue_script('highcharts_sankey', get_template_directory_uri() . '/js/modules/sankey.js', array('highcharts_treemap'), '3.3.4');
		// //
		// wp_enqueue_script('ajax_plugin', plugin_dir_url( __DIR__ ) . '/graficos_chilevalora/js/ajax_plugin.js', array('highcharts_sankey'), '3.3.4');
    //}
...
function dcms_enviar_buscar_ocupacion(){
	...
	$sql_ocupacion = "select id_occupation, name_occupation from cl_occupations where LOWER(name_occupation) LIKE '%".$buscar_ocupaciones."%' order by id_occupation";
    $rs_ocupacion  = $wpdb->get_results($sql_ocupacion);
	...
	if(is_array($rs_ocupacion) && count($rs_ocupacion)>0){
		...
		foreach ($rs_ocupacion as $row) {
			...
			$uri_occupation   	= $row->name_occupation !='' 	  && $row->id_occupation !=''    ? get_site_url().'/ocupacion-detalle/'.$id_occupation : '';
...
function dcms_enviar_buscar_curso(){
	...
	$sql_curso = "
		SELECT 
			...
			price, 
			url, 
			case when modality = 0 then 'Online'
			when modality = 1 then 'Semi-Presencial'
			when modality = 2 then 'Presencial'
			else '' end as modality,  
			skills, 
			duration, 
			offer_origin, 
			description
		FROM cl_courses_offered ".$where.$order_by." limit ".$cant_pagina." offset ".$offset_pag;
	//var_dump($sql_curso);  die;
    $rs_curso  = $wpdb->get_results($sql_curso);

    //var_dump($rs_curso); die;
	...
	if(is_array($rs_curso) && count($rs_curso)>0){
		...
		foreach ($rs_curso as $row) {
			...
			$name_course      	= isset($row->name_course)      	&& $row->name_course !=''   	? $row->name_course : '';
