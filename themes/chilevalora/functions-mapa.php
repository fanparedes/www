    	...
    	foreach ($rs_region as $row_region) {
    		...
    		$sql = "
		    	select
					...
					wp_postmeta.post_id
				from cl_telefonica_nodigitales 
				inner join cl_job_positions on (cl_telefonica_nodigitales.id_occupation = cl_job_positions.id_occupation) and (cl_telefonica_nodigitales.id_region = ".$row_region->id_region.")
				inner join wp_postmeta  on (wp_postmeta.meta_value::int = cl_job_positions.id_occupation::int)
				inner join wp_posts on (wp_posts.ID = wp_postmeta.post_id)
				where post_type = 'detalle_ocupacion' and wp_postmeta.meta_key = 'id' and cl_telefonica_nodigitales.id_region = ".$row_region->id_region."
    ...
    if(is_array($rs_region) && count($rs_region)>0){
    	foreach ($rs_region as $row_region) {
    		...
    		$sql = "
				...
				inner join cl_job_positions_knowledges_offers  on (cl_telefonica_habilidades.code_knowledge::int8 = cl_job_positions_knowledges_offers.code_knowledge::int8)
inner join cl_job_positions on (cl_job_positions_knowledges_offers.code_job_position::varchar = cl_job_positions.id_occupation::varchar)  and (cl_telefonica_habilidades.id_region = ".$row_region->id_region.")
				inner join wp_postmeta  on (wp_postmeta.meta_value::int = cl_job_positions.id_occupation::int)
				inner join wp_posts on (wp_posts.ID = wp_postmeta.post_id)
				where post_type = 'detalle_ocupacion' and wp_postmeta.meta_key = 'id' and cl_telefonica_habilidades.id_region = ".$row_region->id_region."
